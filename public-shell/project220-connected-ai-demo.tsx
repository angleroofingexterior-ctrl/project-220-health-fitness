...
export default function Project220ConnectedAiDemo() {
  ...
  async function runEcosystem() {
    ...
    const events = [
      ...
      `${now} — Smart Basket Optimizer reduced the sample basket by $18.40.`,
      `${now} — Nutrition AI improved basket balance and avoided duplicate creatine.`,
      `${now} — AI Coach prepared a private wellness reminder.`,
      `${now} — Marketplace AI created a unified simulated order.`,
      `${now} — Driver AI generated food-safety and photo-evidence tasks.`,
    ];
    save({
      ...state,
      recipes,
  ...
  if (!loggedIn) {
    return <main style={ui.shell}><form onSubmit={login} style={ui.login}>
      ...
      <p>Master Admin testing account. All results use synthetic demonstration data.</p>
  ...
  return <main style={ui.shell}><section style={ui.app}>
    ...
    <aside style={ui.privacy}><b>Subscriber privacy:</b> Master Admin can operate and test services but cannot open subscriber pantry photos, progress photos, health data, plans or private AI conversations.</aside>
    <section style={ui.hero}>
      <h1>Project 220 AI Operating System</h1>
      <p>Run the connected demo from pantry analysis through recipes, coaching, basket optimization, marketplace simulation and driver safety.</p>
    ...
    <section style={ui.panel}><h2>AI Wellness Coach</h2><p>{state.coach || "Run the ecosystem to generate private coaching guidance."}</p><small>Project 220 provides general wellness education only and does not replace medical care.</small></section>
